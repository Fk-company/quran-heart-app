...
const RadioPage: React.FC = () => {
  ...
  const filtered = search.trim()
    ? stations.filter((s) => s.name.includes(search))
    : stations;
    ...
    <div className="page-container" dir="rtl">
      <div className="px-4 pt-6 max-w-lg mx-auto">
        ...
        {currentTrack && currentTrack.id.startsWith('radio-') && (
          <div className="card-surface mb-4 bg-primary/5 border-primary/20">
            ...
            {filtered.map((station, index) => {
              ...
              return (
                <button
                  ...
                  className={`card-surface w-full flex items-center gap-3 transition-all duration-200 ${
                    isThisPlaying ? 'border-primary/30 bg-primary/[0.03]' : ''
                  }`}
                >
                  ...
                  <div className="flex-1 text-right min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {isThisPlaying ? 'يعمل الآن' : `محطة ${index + 1}`}
                    </div>
                  </div>
                  {isThisPlaying && (
