...
import { BookOpen, Search, Heart, ChevronDown, ChevronUp, Quote, Users, Star, Home, Grid3X3, List, Share2 } from 'lucide-react';
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-2 gap-2">
            {filteredHadiths.map((hadith) => {
              ...
              return (
                <div key={hadith.id} onClick={() => setExpandedId(expandedId === hadith.id ? null : hadith.id)} className="card-surface cursor-pointer">
                  <p className="font-amiri text-sm leading-[1.8] text-foreground line-clamp-2 mb-2">{hadith.text}</p>
                  <div className="flex items-center justify-between">
                    <span className="stat-badge text-[10px]">{cat?.name}</span>
                    <button onClick={(e) => { e.stopPropagation(); shareHadith(hadith); }} className="text-muted-foreground hover:text-primary">
